...
class save_info(object):
    ...
    def dump_lists(self, num_of_steps, num_of_episodes, total_num_episodes, total_num_steps, rewards_std, env_avg_reward, v_loss_list, p_loss_list, eval_avg_reward, eval_avg_reward_std):
    ...
    def save_training_models(self, policy_net, running_state):
        pickle.dump((policy_net, running_state), open(os.path.join(self.assets_dir, self.model_training_saving_path), 'wb'))
    ...
    def save_parameters(self, d_k, max_kl, cg_iter, max_iter, batch_size):
        ...
        field_names= ['max iterations','constraint limit', 'trust region size (max_kl)', 'CG iterations', 'batch size']
        parameters = {'max iterations': max_iter,'constraint limit': d_k, 'trust region size (max_kl)': max_kl, 'CG iterations': cg_iter, 'batch size': batch_size}

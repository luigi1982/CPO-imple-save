...
from utils.constraint_functions import *
import pdb
...
def constraint_cost(state, action):
    """Add your constraint cost function here"""
    if args.env_name == 'CartPole-v1':
        costs = CartPole_pos(state, dtype, device, bound=1)
    if args.env_name == 'LunarLander-v2':
        costs = LunarLander_vel(state, dtype, device)
    if args.env_name == 'BipedalWalker-v3':
        costs = BipedalWalker_avarage_angular_vel(state, dtype, device)
...
def update_params(batch, d_k=0):
    ...
    """perform update"""
    if args.algo_name == "CPO":
        v_loss, p_loss, cost_loss, info = cpo_step(args.env_name, policy_net, value_net, states, actions, returns, advantages, cost_advantages, constraint_value, d_k, args.max_kl, args.damping, args.l2_reg, args.cg_iter, args.save_condition_number)
    ...
    return v_loss, p_loss, cost_loss, constraint_value.item(), info
...
def main_loop():
    ...
    best_avg_training_reward = best_avg_reward
    ...
    for i_iter in range(args.max_iter_num):
        ...
        t0 = time.time()
        if args.algo_name == "CPO":
            v_loss, p_loss, cost_loss, constraint_value, info = update_params(batch, d_k)
        elif args.algo_name == "TRPO":
            v_loss, p_loss, cost_loss, constraint_value = update_params(batch)
        t1 = time.time()
        ...
        constraint_value_list.append(constraint_value)
        info_list.append(info[:-1])
        if args.save_condition_number:
            time_list.append(t1-t0-info[-1])
        else:
        ...
        # save the best model
        if eval_log['env_avg_reward'] >= best_avg_reward: # and eval_log['std_reward'] <= 30: #best_std:
        ...
        # save best model on training data
        if log['env_avg_reward'] >= best_avg_training_reward:
            print('Saving new best training model !!!!')
            to_device(torch.device('cpu'), policy_net, value_net)
            save_info_obj.save_training_models(policy_net, running_state)
    ...
    save_info_obj.dump_lists(num_of_steps, num_of_episodes, total_num_episodes, total_num_steps, rewards_std, env_avg_reward, v_loss_list, p_loss_list, eval_avg_reward, eval_avg_reward_std)
    save_info_obj.save_info(eval_avg_reward, env_avg_reward, constraint_value_list, info_list, time_list)
    save_info_obj.save_parameters(d_k, args.max_kl, args.cg_iter, args.max_iter_num, args.min_batch_size)
